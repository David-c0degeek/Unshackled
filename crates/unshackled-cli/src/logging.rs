...
//!
//! * Default — no `UNSHACKLED_LOG` set: logging follows `RUST_LOG` and writes to
...
//!   stderr diagnostics.
//! * File — `UNSHACKLED_LOG` set to a filter (e.g. `debug`): logs are written to
//!   a per-run file under `<cwd>/.unshackled/logs/` and nothing is emitted to the
...
const ENV_VAR: &str = "UNSHACKLED_LOG";
...
fn init_file(cwd: &Path, value: &str) -> Option<PathBuf> {
    let dir = cwd.join(".unshackled").join("logs");
...
/// Build the env-filter directive string for a `UNSHACKLED_LOG` value.
...
mod tests {
    ...
    fn explicit_targets_pass_through_unchanged() {
        let value = "unshackled_llm=trace,unshackled_harness=debug";
        assert_eq!(scoped_filter(value), value);
    }

    #[test]
    fn log_file_name_is_unique_per_stamp_and_ends_in_log() {
        assert_eq!(log_file_name(42), "unshackled-42.log");
