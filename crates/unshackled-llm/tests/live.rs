...
//! `UNSHACKLED_LIVE_TESTS` and `OPENAI_API_KEY` are set, so it never runs in
//! default CI and needs no credentials by default (docs/08 Live Tests).

use futures::StreamExt;
use unshackled_core::{Message, Role, Secret};
use unshackled_llm::{ModelEvent, ModelProvider, ModelRequest, OpenAiProvider, SourceType};

#[tokio::test]
async fn live_openai_text_completion() {
    if std::env::var("UNSHACKLED_LIVE_TESTS").is_err() {
        eprintln!("skipping live test: set UNSHACKLED_LIVE_TESTS to enable");
        return;
    }
