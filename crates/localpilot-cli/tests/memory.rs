...
fn memory_inspect_delete_and_disable() {
    ...
    assert!(localpilot_localmind::memory_list(dir.path())
        .unwrap()
        .is_empty());

    // Disable stops injection.
    run(dir.path(), &["memory", "disable"]);
    assert!(!localpilot_localmind::memory_injection_enabled(dir.path()));
    assert!(localpilot_localmind::context_for(dir.path(), "parser")
        .unwrap()
        .is_none());
}
...
fn promoted_memory(dir: &std::path::Path, lesson: &str) -> String {
    ...
    store
        ...
        .unwrap();
    localpilot_localmind::closeout_session(dir, &store, session).unwrap();
    let item = localpilot_localmind::review_list(dir)
        .unwrap()
        .into_iter()
        .find(|item| item.summary == lesson)
        .unwrap();
    localpilot_localmind::review_decide(dir, &item.id, ReviewVerdict::Accept, "test", None)
        .unwrap();
    localpilot_localmind::promote(dir, &item.id).unwrap()
}
