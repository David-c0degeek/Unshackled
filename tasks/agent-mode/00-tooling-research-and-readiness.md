...
- [ ] **00.2** (agent) Inventory the agent-mode surfaces this plan touches:
...
- [ ] **00.3** (agent) Run the local gate (`fmt --check`, clippy, per-crate
      tests, check) and record the baseline green state and any windows-gnu
      caveats.
- [ ] **00.4** (agent) Research current best practices for agentic tool-use
...
- [ ] **00.5** (agent) Confirm the documented public env-var/header conventions
      to support (Anthropic `ANTHROPIC_BASE_URL`/`ANTHROPIC_API_KEY`; OpenAI
      `OPENAI_BASE_URL`/`OPENAI_API_KEY`) with links and versions.
- [ ] **00.6** (agent) Decide which assistant skills / MCP servers / local tools
      aid this work; classify `adopt`/`defer`/`reject` with rationale.
- [ ] **00.7** (agent) Set up only approved tooling; keep secrets out of repo
      config.
- [ ] **00.8** (agent) Bake findings into the plan (§6 principles, §7 gates,
